...
public class AddPlaylistPage extends JFrame {
	...
	private void initComponents() {
		...
		layout.setHorizontalGroup(layout.createParallelGroup().addComponent(errorMessage)
				.addGroup(layout.createSequentialGroup()
						.addGroup(layout.createParallelGroup().addComponent(playlistNameLabel).addComponent(songLabel)
								.addComponent(addSongButton).addComponent(addPlaylistButton)))
				.addGroup(layout.createParallelGroup().addComponent(playlistNameTextField).addComponent(songList))
				.addGroup(layout.createParallelGroup().addComponent(songScrollPane, 200, 200, 400)));

		// rows
		layout.setVerticalGroup(layout.createParallelGroup().addComponent(errorMessage)
				.addGroup(layout.createSequentialGroup()
						.addGroup(layout.createSequentialGroup().addComponent(playlistNameLabel)
								.addComponent(playlistNameTextField))
				.addGroup(layout.createSequentialGroup().addComponent(songLabel).addComponent(songList))
				.addComponent(songScrollPane, 200, 200, 400).addGroup(layout.createSequentialGroup()
						.addComponent(addSongButton).addComponent(addPlaylistButton))));
		pack();
	}
	...
	private void addSongButtonActionPerformed(java.awt.event.ActionEvent evt) {
		DefaultTableModel model = (DefaultTableModel) songsTable.getModel();
		model.addRow(new Object[] { selectedSong, songs.get(selectedSong).getTitle(),
